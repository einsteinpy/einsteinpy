...
# Intensity Maps -
# Allowing for intensity calculations
# Obtaining BH Shadows
# Showcasing Intensity Shifts -> Leverages Plotting module for this
# Keplerian Disk, showcasing Lensing Effect and Intensity Shift.

# 3 modules will be interdependent :
# `geodesic` | `radiative` | `plotting.geodesic` | `plotting.radiative`

# `geodesic.py`
# `Geodesic` class -> `Timelike` & `Nulllike` @classmethods
# -> Nulllike:
# One, like Timelike
# Other, NullBundle:
# Will have camera controls (impact parameters) and will set up photon_0 (initial photon vector)
# Does all transformations to BL or KS and calls `integrate` in `metric.Kerr` or `metric.Schwarzschild`
# Transforms back to camera coordinates (impact parameters)
# Data structure for storing a trajectory/path in the spacetime for time/null particles
# Also has `NullGeodesicBundle` to declare a performant, Numpy leveraging,...
# ...set of geodesics (they lose out on some things, like individual attributes) for radiative transfer applications
# Has interfaces to `plotting.geodesic`

# `rays` -> `radiative.py` (Top Level Module)
# Has `intensity`
# Has utils `optical_depth` and `intensity` calculations along a single geodesic.
# Has interfaces to `plotting.radiative`

# `plotting.geodesic` -> `plotting.geodesic`
# Plots individual Geodesic
# Plots multiple Geodesics
# `plotting.rays` -> `plotting.radiative`
# Plots BH Shadow
# Plots IntensityMap (Plot)
# Plots Keplerian Disc (which shows Lensing and Intensity Shift/Redshift)


# `geodesic` will have camera controls and will set up photon_0 (initial photon vector)
# -> does all transformations to BL or KS and calls integrate in `metric`
# -> then transforms back to camera coordinates (impact parameters)
# -> calls plotting.geodesic.GeodesicPlot() to plot Geodesics
# -> leverages radiative.py to do intensity calculations

# -> calls plotting.radiative.IntensityPlot() to plot Intensity
# Has options:
# Contour Plot / Color Map (Intensity variation radially outward from BH)

# The stored geodesic will be used by
