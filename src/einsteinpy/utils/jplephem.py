...
def _get_bary_posvel(body, time, ephemeris=None, get_velocity=True):
    ...
    if kernel is None:
        ...
        if body == "earth":
            ...
            if get_velocity:
                raise KeyError(
                    "the Moon's velocity cannot be calculated with "
            ...
            if body == "sun":
                ...
                try:
                    ...
                    raise KeyError(
                        ...
                        "calculated with the '{1}' ephemeris.".format(body, ephemeris)
                    )
        ...
        if isinstance(body, str):
            ...
            try:
                ...
                raise KeyError(
                    "{0}'s position cannot be calculated with "
                    "the {1} ephemeris.".format(body, ephemeris)
                )
        else:
            # otherwise, assume the user knows what their doing and
            # intentionally passed in a kernel chain
            kernel_spec = body

        # jplephem cannot handle multi-D arrays, so converting to 1D here.
...
def get_body_barycentric_posvel(body, time, ephemeris=None):
    ...
    """
    return _get_bary_posvel(body, time, ephemeris)
...
def get_body_barycentric(body, time, ephemeris=None):
    ...
    position : `~astropy.coordinates.CartesianRepresentation`
        ...
        """
    return _get_bary_posvel(body, time, ephemeris, get_velocity=False)


def _get_body_helio_posvel(body, time, ephemeris=None, get_velocity=True):
    if get_velocity:
        body_pos_bary, body_vel_bary = _get_bary_posvel(
            body, time, ephemeris, get_velocity
        )
        sun_pos_bary, sun_vel_bary = _get_bary_posvel(
            "sun", time, ephemeris, get_velocity
        )
