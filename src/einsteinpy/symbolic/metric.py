...
class Metric(AbstractTensor, TensorIndexType):
    ...
    def christoffel(self):
        if self._christoffel is None:
            ...
            d = self.partial
            g = self.metric
            gamma = (1/2)*g(si,rh)*(d(-mu)*g(-nu,-rh) + d(-nu)*g(-rh,-mu) - d(-rh)*g(-mu,-nu))
    ...
    def riemann(self):
        if self._riemann is None:
            ...
            R = d(-mu)*gamma(rh,-nu,-si) - d(-nu)*gamma(rh,-mu,-si) \
                + gamma(rh,-mu,-la)*gamma(la,-nu,-si) - gamma(rh,-nu,-la)*gamma(la,-mu,-si)
            res = expand_tensor(R)
            self._riemann = Tensor('R', res, self, symmetry=[[2, 2]], covar=(1,-1,-1,-1))
    ...
    def weyl(self):
        if self._weyl is None:
            n = self.dim
            if n < 3:
                raise ValueError(f'the Weyl tensor is only defined in dimensions of 3 or more. {self} is of dimension {n}')
            elif n == 3:
            ...
            g = self.metric
            C = R(rh,-si,-mu,-nu) \
                - c1*(g(rh,-mu)*RR(-nu,-si) - g(rh,-nu)*RR(-mu,-si) + g(-si,-nu)*RR(-mu,rh) - g(-si,-mu)*RR(-nu,rh)) \
