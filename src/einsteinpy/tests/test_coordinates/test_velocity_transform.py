...
def cartesian_differential():
    return coordinates.CartesianDifferential(
        10/np.sqrt(2) * u.km, 10/np.sqrt(2) * u.km, 0 * u.km,
...
def spherical_differential():
    return coordinates.SphericalDifferential(
        10.0 * u.km, 1.5707963267948966 * u.rad, 0.7853981633974483 * u.rad,
        10.0 * u.km / u.s,
        -20.0 * u.rad / u.s,
        20.0 * u.rad / u.s
    )
...
def assert_cartesian_differential(cartesian_differential, to_cartesian_differential):
    ...
    assert abs(to_cartesian_differential.v_x - cartesian_differential.v_x) <= (1e-5) * u.km / u.s
    assert abs(to_cartesian_differential.v_y - cartesian_differential.v_y) <= (1e-5) * u.km / u.s
    assert abs(to_cartesian_differential.v_z - cartesian_differential.v_z) <= (1e-5) * u.km / u.s
...
def assert_spherical_differential(spherical_differential, to_spherical_differential):
    ...
    assert abs(to_spherical_differential.theta - spherical_differential.theta) <= (1e-5) * u.rad
    assert abs(to_spherical_differential.phi - spherical_differential.phi) <= (1e-5) * u.rad
    assert abs(to_spherical_differential.v_r - spherical_differential.v_r) <= (1e-5) * u.km / u.s
    assert abs(to_spherical_differential.v_t - spherical_differential.v_t) <= (1e-5) * u.rad / u.s
    assert abs(to_spherical_differential.v_p - spherical_differential.v_p) <= (1e-5) * u.rad / u.s
...
def test_CartesianToSphericalDifferential(cartesian_differential, spherical_differential):
...
def test_CartesianToBoyerLindquistDifferential(cartesian_differential, boyerlindquist_differential):
...
def test_SphericalToCartesianDifferential(spherical_differential, cartesian_differential):
...
def test_SphericalToBoyerLindquistDifferential(spherical_differential, boyerlindquist_differential):
    bl_differential, a = boyerlindquist_differential
    to_bl_differential = spherical_differential.boyerlindquist_differential(a)
    assert_boyerlindquist_differential(bl_differential, to_bl_differential)

def test_BoyerLindquistToCartesianDifferential(boyerlindquist_differential, cartesian_differential):
    bl_differential, a = boyerlindquist_differential
    to_cartesian_differential = bl_differential.cartesian_differential(a)
    assert_cartesian_differential(cartesian_differential, to_cartesian_differential)

def test_BoyerLindquistToSphericalDifferential(boyerlindquist_differential, spherical_differential):
    bl_differential, a = boyerlindquist_differential
