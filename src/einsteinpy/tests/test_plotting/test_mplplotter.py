...
from einsteinpy.plotting import MatplotlibPlotter
...
def test_matplotlibplotter_has_axes(dummy_data):
    geodesic = dummy_data
    cl = MatplotlibPlotter()
    assert isinstance(cl.ax, Axes)
    assert cl.attractor_present is False


@mock.patch("einsteinpy.plotting.mplplotter.plt.show")
...
@mock.patch("einsteinpy.plotting.mplplotter.plt.savefig")
...
def test_plot_calls_draw_attractor_Manualscale(dummy_data):
    geodesic = dummy_data
    cl = MatplotlibPlotter(attractor_radius_scale=1500)
    cl.plot(geodesic)
    assert cl.attractor_present
